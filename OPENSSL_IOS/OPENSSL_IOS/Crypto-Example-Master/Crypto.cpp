...
int Crypto::rsaEncrypt(const unsigned char *message, size_t messageLength, unsigned char **encryptedMessage, unsigned char **encryptedKey,
  ...
  *encryptedMessage = (unsigned char*)malloc(messageLength + EVP_MAX_IV_LENGTH);
  ...
  cout << "ecrypted key = [" << *encryptedKey << " ]" << endl;
  cout << "remotePublicKey = [" << remotePublicKey << " ]" << endl;
    cout << "n = [ ";
    cout << BN_bn2dec(remotePublicKey->pkey.rsa->n);
//    for (int i = 0; i < remotePublicKey->pkey.rsa->n->dmax; i++) {
//         cout << (unsigned long long)(remotePublicKey->pkey.rsa->n->d[i]);
//    }
    cout  << " ]" << endl;
    
    cout << "p = [ ";
    cout << BN_bn2dec(remotePublicKey->pkey.rsa->p);
//    for (int i = 0; i < remotePublicKey->pkey.rsa->p->dmax; i++) {
//        cout << (remotePublicKey->pkey.rsa->p->d[i]);
//    }
    cout  << " ]" << endl;
    
    cout << "q = [ ";
    cout << BN_bn2hex(remotePublicKey->pkey.rsa->q);
//    for (int i = 0; i < remotePublicKey->pkey.rsa->q->dmax; i++) {
//        cout << (unsigned long long)(remotePublicKey->pkey.rsa->q->d[i]);
//    }
    cout  << " ]" << endl;
    cout << "e = [ " << BN_bn2dec(remotePublicKey->pkey.rsa->e) << " ]" << endl;
    cout << "d = [ " << BN_bn2dec(remotePublicKey->pkey.rsa->d) << " ]" << endl;
    cout << "dmp1 = [ " << BN_bn2dec(remotePublicKey->pkey.rsa->dmp1) << " ]" << endl;
    cout << "dmq1 = [ " << BN_bn2dec(remotePublicKey->pkey.rsa->dmq1) << " ]" << endl;
    cout << "iqmp = [ " << BN_bn2dec(remotePublicKey->pkey.rsa->iqmp) << " ]" << endl;
...
bool Crypto::generate_key_files() {
    int             ret = 0;
    RSA             *r = NULL;
    BIGNUM          *bne = NULL;
